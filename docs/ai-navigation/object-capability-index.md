...
| `k.site.labels` | 内容系统 | [标签与 HTML Block](/content-system/labels-and-html-blocks) | `getValue` `getOrAdd` | 不是 `k.content` 内容文件夹 |
| `k.site.htmlBlocks` | 内容系统 | [标签与 HTML Block](/content-system/labels-and-html-blocks) | `getValue` `update` | 不是普通模板片段文件 |
| `k.content.Blog` | 内容系统 | [内容类型与数据类型](/content-system/content-types-and-data-types) | 文件夹实例、内容查询 | 不等于 Type 定义本身 |
| `Commerce Settings` | 内容系统 | [电商配置与内容关联](/content-system/commerce-and-settings) | 商品字段、配置对象 | 不只是 KScript API |
...
const lookup = {
  ...
  "k.site.labels": "/content-system/labels-and-html-blocks",
